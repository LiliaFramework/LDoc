...
local function setup_kinds()
   ...
   for k in pairs(kind_names) do
      if not known_types[k] then quit("unknown item type " .. tools.quote(k) .. " in kind_names") end
   end
end
...
local ldoc_contents = {'alias', 'add_language_extension', 'custom_tags', 'new_type', 'add_section', 'tparam_alias', 'file', 'project', 'title', 'package', 'format', 'output', 'dir', 'ext', 'topics', 'one', 'style', 'template', 'description', 'examples', 'pretty', 'charset', 'plain', 'readme', 'all', 'manual_url', 'ignore', 'colon', 'sort', 'module_file', 'vars', 'boilerplate', 'merge', 'wrap', 'not_luadoc', 'template_escape', 'merge_error_groups', 'no_return_or_parms', 'no_summary', 'full_description', 'backtick_references', 'custom_see_handler', 'no_space_before_args', 'simple_args_string', 'parse_extra', 'no_lua_ref', 'sort_modules', 'use_markdown_titles', 'unqualified', 'custom_display_name_handler', 'kind_names', 'custom_references', 'strip_metamethod_prefix', 'dont_escape_underscore', 'global_lookup', 'prettify_files', 'convert_opt', 'user_keywords', 'no_viewed_topic_at_top', 'postprocess_html', 'use_new_templates', 'pretty_urls', 'pretty_topic_names', 'custom_css', 'version', 'no_args_infer'}
...
local function read_ldoc_config(fname)
   ...
   local chunk, err, _
   if args.filter == 'none' then print('reading configuration from ' .. fname) end
   ...
   for k in pairs(ldoc) do
      if not ldoc_contents[k] then quit("this config file field/function is unrecognized: " .. k) end
   end
...
if args.module then
   ...
   if args.file:match'^%a+$' and global.functions[args.file] then args.file = 'global.' .. args.file end
...
if args.file == '.' then
   ...
   if args.file == nil then lapp.error('missing required parameter: file') end
   args.file = abspath(args.file)
end
...
if type(source_dir) == 'table' then source_dir = source_dir[1] end
if type(source_dir) == 'string' and path.isfile(source_dir) then source_dir = path.splitpath(source_dir) end
...
local function process_file(f, flist)
   ...
   if ftype then
      ...
      if err then
         if F then F:warning("internal LDoc error") end
         quit(err)
      end
...
if ldoc.parse_extra and ldoc.parse_extra.C then ldoc.no_lua_ref = true end
if ldoc.merge_error_groups == nil then ldoc.merge_error_groups = 'Error Message' end
...
local function reorder_module_file()
   if ldoc.module_file then
      ...
      return function(x, y) return mf[x] and not mf[y] end
   end
end
...
elseif path.isdir(args.file) then
   ...
   if not config_dir then
      ...
      local config_files = files:filter(function(f) return path.basename(f) == args.config end)
      if #config_files > 0 then
         config_dir = read_ldoc_config(config_files[1])
         if #config_files > 1 then print('warning: other config files found: ' .. config_files[2]) end
      end
   end
...
elseif path.isfile(args.file) then
   ...
   if not config_dir then
      ...
      if path.isfile(config) then read_ldoc_config(config) end
   end
...
local function add_special_project_entity(f, tags, process, pretty)
   ...
   if pretty then tags.name = tags.name:gsub("%..+$", "") end
   local text = utils.readfile(f)
   local item = F:new_item(tags, 1)
   if process then text = process(F, text) end
   F:finish()
   file_list:append(F)
   item.body = text
   return item, F
...
if type(ldoc.examples) == 'string' then ldoc.examples = {ldoc.examples} end
if type(ldoc.examples) == 'table' then prettify_source_files(ldoc.examples, "example") end
...
if ldoc.prettify_files then
   ...
   elseif type(ldoc.prettify_files) == 'string' then
      ...
      if ldoc.prettify_files == 'show' then
         ...
         if path.exists'show' then print("Notice: if you only want to prettify files in `show`, then set prettify_files to `show/`") end
      else
         files = tools.expand_file_list({ldoc.prettify_files}, '*.*')
      end
   end
...
if ldoc.sort_modules then table.sort(module_list, function(m1, m2) return m1.name < m2.name end) end
...
if args.module then
   ...
   if args.module == true then
      ...
      if not fun then fun = M.items.by_name[M.mod_name .. ':' .. name] end
...
if args.ext == 'md' then
   if #module_list ~= 1 then quit("can currently only generate Markdown output from one module only") end
   if not ldoc.template or ldoc.template == '!' then ldoc.template = '!md' end
...
style_dir'style'
style_dir'template'
if not args.ext:find'^%.' then args.ext = '.' .. args.ext end
if args.one then ldoc.style = '!one' end
...
if builtin_style or builtin_template then
   ...
   if not path.isdir(tmpdir) then lfs.mkdir(tmpdir) end
   local function tmpwrite(name)
      local ok, text = pcall(require, 'ldoc.html.' .. name:gsub('%.', '_'))
      if not ok then quit("cannot find builtin template " .. name .. " (" .. text .. ")") end
      if not utils.writefile(path.join(tmpdir, name), text) then quit("cannot write to temp directory " .. tmpdir) end
   end

   if builtin_style then
      if builtin_style ~= '' then ldoc.css = 'ldoc_' .. builtin_style .. '.css' end
      tmpwrite(ldoc.css)
      args.style = tmpdir
   end

   if builtin_template then
      if builtin_template ~= '' then ldoc.templ = 'ldoc_' .. builtin_template .. '.ltp' end
      tmpwrite(ldoc.templ)
...
if args.verbose then
   print'modules'
   for k in pairs(module_list.by_name) do
      print(k)
   end
end

if args.fatalwarnings and Item.had_warning then os.exit(1) end
